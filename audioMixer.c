...
Date: 2023-02-25
Subject: Implementation of the AudioMixer module to get the current accelerometer action
*/
...
// Queue the sound bite pSound into soundBites buffer to be played ASAP
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	int slot_num = -1;
	// Find the first free slot to insert *pSound into
	for (int i = 0; i < MAX_SOUND_BITES; i++) {
	...
	if (slot_num == -1) {
		printf("ERROR: Cannot queue sound as the buffer is full\n");
		return;
	}
	
	// Insert pSound into the queue
	soundBites[slot_num].pSound = pSound;
	soundBites[slot_num].location = 0; // has not been played yet
...
void AudioMixer_cleanup(void)
{
	...
	// Free the remaining soundBite memory 
	for(int i=0; i < MAX_SOUND_BITES; i++)
	{
		if (soundBites[i].pSound != NULL)
		{
			if(soundBites[i].pSound->pData != NULL) {
				free(soundBites[i].pSound->pData);
			}
...
//    `buff`: buffer to fill with new PCM data from sound bites. --> i.e. the playbackBuffer
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	// 1. Wipe the playbackBuffer to all 0's to clear any previous PCM data
	// FIXED - Should  wipe playbackBuffer, not buff
		// Previously was: "buff" and  "size * SAMPLE_SIZE"
	memset(playbackBuffer, 0, playbackBufferSize * SAMPLE_SIZE);

	// 2.synchronize the function
	pthread_mutex_lock(&audioMutex);
	// 3. Loop soundBites
	for (int i = 0; i < MAX_SOUND_BITES; i++)
	{
		...
		// Extract the current pSound (index i) from soundBites
		short *currentSound = soundBites[i].pSound->pData;
		int index_currentSound = soundBites[i].location;
		int numSamples_currentSound = soundBites[i].pSound->numSamples;

		// REFACTORED - Previously was in the inner for loop
		// Check if the current sound is already played completely
		if (index_currentSound == numSamples_currentSound) {
			// Free the already played sound
		...
		// Write into the appropriate position of the buff
		for (int j = 0; (j < size) && (index_currentSound < numSamples_currentSound); j++)
		{
			//
			int added_value = ((int)buff[j]) + ((int)currentSound[index_currentSound]);
			if (added_value >= SHRT_MAX)
			{
				added_value = SHRT_MAX;
			}
			else if (added_value <= SHRT_MIN)
			{
				added_value = SHRT_MIN;
			}
			buff[j] = (short)added_value;
			index_currentSound++;
		}

		soundBites[i].location = index_currentSound;
	}
